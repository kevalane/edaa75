...
(defn insert-into-queue
    ...
    (def allBelowList (for [q Q :when (< (:frequency q) (:frequency T))]
        q
    ))
    (def allAbove (for [q Q :when (>= (:frequency q) (:frequency T))]
        q
    ))
...
(defn create-tree
    ...
    ;; YOUR CODE HERE
    (if (= (count Q) 1)
        (first Q) ;; step a, as described
        (
            (def stepB (make-branch (first Q) (second Q) (+ (:frequency (first Q)) (:frequency (second Q))))) ;; self explanatory with b
            ;; make branch takes 2 or 3 parameters, we use the one with three to add the freq too.
            (def newQ (drop 2 Q)) ;; thanks for tip about drop, newQ now doesnt hold the two old ones
            (create-tree (insert-into-queue stepB newQ)) ;; recursive call inline with order (d)
        )
    )
