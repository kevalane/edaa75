...
(defn create-square-sum-relation
    ...
    [A]
    ;; https://clojuredocs.org/clojure.core/for for parenthesis and more on for loop
    (set ;; make everything from for loop a set 
        (for [a A b A ;; double loop over all
            :let [y [a b]] ;; define y as the tuple
            :when (square? (+ a b))] ;; check if square and only take those
            y ;; append the y to set
        )
    )
...
(defn- H'
    ...
    ;;(defn newS [x] (disj S x)) ;; , yields our 'new' S --> works (disj #{1 2 3} 3) yields #{1 3}

    (if (empty? S) ;; do we have anywhere to go? no line 1, yes line 2 onwards
        (concat P [a]) ;; check comment, append the one, no more bc S is empty
        (let [possible (image-of E a)] ;; all possible nodes to go to, image-of like always
            (let [useful (intersection S possible)] ;; but S tells us where we can go, thus 'useful'
                (let [travelled (concat P[a])] ;; this replaces '() as P in defn H function call
                    (some ;; equivalent to for every useful, apply anonymous function
                        #(H' E % (disj S %) travelled) ;; just copy pasted from H, recursive call
                    useful)
                )
            )
        )
...
(defn next-positions
    ...
    (defn calcMoves [x] (add-move pos x))
    (def mapOfMoves (map #(calcMoves %) Moves))
    (def setOfMoves (set mapOfMoves))
    (intersection B setOfMoves)
...
(defn create-knights-move-relation
    ...
    [B]
    (let [x ;; lets make an a with all moves
        (for [a B b (next-positions a B)] [a b])]
        (set x)
    )
