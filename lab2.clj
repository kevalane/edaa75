...
(defn no-links? 
  ...
  ;; if no outgoing, it should not be element of dom

  (not (contains? (dom web) page))
)
...
(defn random-link 
  ...
  [web page]
  (def x (image-of web page)) ;; stores all outgoing in a set
  (def x (seq x)) ;; convert to set
  (rand-nth x) ;; return a random one
...
(defn surf 
  ...
  (if (< (rand 1) 0.1)
    (random-page web) ;; with probability 0.1 go to random page
    (if (no-links? web current) ;; this happens with prob 0.9
      (random-page web) ;; if no links, we go to random page 
      (random-link web current) ;; else we go to a random link
    )
  )
;; hint: check out "rand"
